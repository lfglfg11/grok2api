...
- 图片编辑结果查询在诊断日志中记录候选图的顺序与短摘要哈希，用于区分“上游忽略参考图”和“适配器选错候选图”；不记录完整媒体 URL。
...
- Statsig verification meta 拉取会附加一次性 `__grok2api_statsig` 缓存破坏参数；该参数只用于获取最新 `/imagine` HTML，不会进入签名目标路径或上游业务载荷，避免出口/CDN复用旧页面导致连续 `403 code=7`。
