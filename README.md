...
The Web flow retains all three browser-captured phases: upload uses the `/imagine` page, edit creation uses an `/imagine/post/<UUID>` Referer, and the gateway then reads `/rest/app-chat/conversations/<conversation-id>/responses?conversationKind=CONVERSATION_KIND_IMAGINE` with the matching post/conversation Referer. Final image URLs from this conversation response take priority over incidental `/generated/` candidates in the creation stream. Diagnostic logs also record only the returned model labels and aggregate reference counts, making it possible to distinguish the selected Imagine generation without logging asset identifiers or image content.
