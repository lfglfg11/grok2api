...
When one public image model exposes both generation and edit routes, Chat Completions and Responses select by input: text-only requests use image generation, while a current user message containing image attachments uses the edit route. For `grok-imagine-image-2.0`, the public model name is preserved at the gateway, but the Web edit request is mapped exactly to the captured protocol (`modelName=imagine-image-edit`, `imageEditModel=imagine`, and `mediaGenInput.imageToImage`), preventing accidental fallback to the first-generation image route. Images Edits continues to select the edit capability independently. Fixed `-2k` aliases are recovered from the original public model name in Chat requests, so generation and edit responses retain the same 2K post-processing contract.
...
Before a Web image edit upload, the adapter refreshes the current `user_id` from `/api/auth/session` on the same Web lease and uses it for `x-userid` plus the upload/generation flow. The persisted account identity is only a fallback when the Session refresh is unavailable, preventing an uploaded asset from being accepted but treated as non-root-owned by the upstream editor. A boolean-only diagnostic checks whether the returned asset URI belongs to that runtime user.

Image-edit upload and generation requests also carry the same runtime identity cookies: the resolved `x-userid` plus a stable per-account `grok_device_id`. When an account is imported or updated with a browser-captured `grok_device_id`, both phases reuse that exact durable device identity; legacy accounts without one retain the deterministic UUID derived from `x-userid`. In managed Clearance mode, only the account device cookie is merged with the server-refreshed Cloudflare cookies, so FlareSolverr/on-demand renewal remains authoritative for `cf_clearance`.

The Web flow retains all three browser-captured phases: upload uses the `/imagine` page, edit creation uses an `/imagine/post/<UUID>` Referer, and the gateway then reads `/rest/app-chat/conversations/<conversation-id>/responses?conversationKind=CONVERSATION_KIND_IMAGINE` with the matching post/conversation Referer. The final image is selected only from `generatedImageUrls` on the last Imagine assistant response; input attachments, preview images, and incidental `/generated/` strings are not eligible. Diagnostic logs also record only the returned model labels and aggregate reference counts, making it possible to distinguish the selected Imagine generation without logging asset identifiers or image content.

Generic Web media POST requests derive `x-statsig-id` from the normal Web verification flow; an upstream code 7 “reload” response invalidates and retries only the matching signature. The verification document fetch includes a one-time cache-busting query parameter so an intermediary cannot replay an older page; this parameter is used only for fetching HTML and is not included in the signed upstream path or business payload.
