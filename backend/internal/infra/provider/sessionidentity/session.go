...
func FetchWithLease(ctx context.Context, baseURL, token string, lease *infraegress.Lease, egress *infraegress.Manager) (provider.AccountIdentity, error) {
	...
	// Grok may issue the browser-bound device identity lazily from the
	// Session endpoint. Keep it on this lease so the immediately following
	// upload/create/responses calls use the same identity instead of the
	// deterministic fallback generated by the Web provider.
	setCookies := response.Cookies()
	mergeSessionDeviceCookie(lease, setCookies)
	if names := sessionCookieNames(setCookies); len(names) > 0 {
		slog.Default().Info("web_session_set_cookie", "names", names)
	}
...
func sessionCookieNames(cookies []*http.Cookie) []string {
...
func mergeSessionDeviceCookie(lease *infraegress.Lease, cookies []*http.Cookie) {
	if lease == nil {
		return
	}
	for _, cookie := range cookies {
		if cookie == nil || cookie.Name != "grok_device_id" || strings.TrimSpace(cookie.Value) == "" {
			continue
		}
		lease.CFCookies = mergeCookie(lease.CFCookies, cookie.Name, cookie.Value)
	}
}

func mergeCookie(raw, name, value string) string {
