...
// FetchVideoInputImage uses managed public-asset egress without Provider credentials.
...
func newConsoleMediaUpstreamError(status int, body []byte, retryAfter time.Duration) error {
	...
	upstreamErr := &consoleMediaUpstreamError{
	...
	if isConsoleVideoInputDownloadError(body) {
		return errors.Join(provider.ErrVideoInputDownload, upstreamErr)
	}
	return upstreamErr
}
