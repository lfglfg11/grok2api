...
func isOpenAIImageCompatibilityModel(model string) bool {
	value := strings.ToLower(strings.TrimSpace(model))
	value = strings.TrimPrefix(value, "console/")
	return strings.HasPrefix(value, "gpt-image-") || strings.HasPrefix(value, "dall-e-")
}
...
func (h *Handler) generateImage(c *gin.Context) {
	...
	if quality == "high" && isOpenAIImageCompatibilityModel(request.Model) {
		// OpenAI image clients commonly send high; the quality model alias already
		// selects the higher-quality upstream route, so leave the Console knob unset.
		quality = ""
	}
...
func (h *Handler) editImage(c *gin.Context) {
	...
	model, prompt := strings.TrimSpace(request.Model), strings.TrimSpace(request.Prompt)
	count := 1
	if request.Count != nil {
		count = *request.Count
	}
	...
	quality := strings.ToLower(strings.TrimSpace(request.Quality))
	if quality == "high" && isOpenAIImageCompatibilityModel(model) {
		quality = ""
	}
	if quality != "" && quality != "low" && quality != "medium" {
		writeOpenAIError(c, http.StatusBadRequest, "invalid_parameter", "quality 必须是 low 或 medium")
	...
	result, err := h.gateway.EditImage(c.Request.Context(), gateway.ImageEditInput{RequestID: requestID, ClientKey: clientKey, PublicModel: model, Prompt: prompt, ImageURLs: imageURLs, Count: count, AspectRatio: aspectRatio, Resolution: resolution, Quality: quality, ResponseFormat: request.ResponseFormat, Streaming: request.Stream, PartialImages: partialImages, Method: c.Request.Method, Path: c.Request.URL.Path, Headers: c.Request.Header.Clone()})
	if err != nil {
		writeGatewayError(c, err)
		return
	}
...
func (h *Handler) createCompatibleVideo(c *gin.Context) {
	...
	imageURL, references, err := normalizeCompatibleVideoImages(request)
	if err != nil {
		writeOpenAIError(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
	...
	if prompt == "" && imageURL == "" {
		writeOpenAIError(c, http.StatusBadRequest, "invalid_request", "文本生视频和参考图视频必须提供 prompt；首帧图片生视频可以省略 prompt")
		return
	}
	...
	job, err := h.gateway.CreateVideo(c.Request.Context(), gateway.VideoInput{
		RequestID: requestID, ClientKey: clientKey, PublicModel: model, Compatibility: true,
		Prompt: prompt, Duration: duration, AspectRatio: aspectRatio, Resolution: resolution,
		ImageURL: imageURL, ReferenceURLs: references,
	})
	if err != nil {
		writeGatewayError(c, err)
		return
	}
	c.JSON(http.StatusOK, compatibleVideoResponse(job, h.videoPlaybackURL(job)))
}

func normalizeCompatibleVideoImages(request compatibleVideoRequest) (string, []string, error) {
	image := strings.TrimSpace(request.Image)
	inputReference := strings.TrimSpace(request.InputReference)
	if image != "" && inputReference != "" {
		return "", nil, errors.New("image 与 input_reference 不能同时提供")
	}
	if image == "" {
		image = inputReference
	}
	if len(request.Images) > 0 && len(request.ReferenceImages) > 0 {
		return "", nil, errors.New("images 与 reference_images 不能同时提供")
	}
	references := request.ReferenceImages
	if len(references) == 0 {
		references = request.Images
	}
	if image != "" && len(references) > 0 {
		return "", nil, errors.New("image/input_reference 不能与 images/reference_images 同时提供")
	}
...
func aspectRatioFromVideoSize(value string) (string, error) {
	...
	if strings.Contains(normalized, ":") || strings.Contains(normalized, "/") {
		...
		if len(parts) != 2 {
			return "", errors.New("size 必须是 16:9、9/16 或 WIDTHxHEIGHT")
		}
	...
	if len(parts) != 2 {
		return "", errors.New("size 必须是 WIDTHxHEIGHT 或 WIDTH:HEIGHT")
	}
	width, err1 := strconv.Atoi(parts[0])
	height, err2 := strconv.Atoi(parts[1])
	if err1 != nil || err2 != nil || width < 1 || height < 1 || width > 16384 || height > 16384 {
		return "", errors.New("size 必须包含合理的正数尺寸")
	}
	divisor := gcd(width, height)
	return strconv.Itoa(width/divisor) + ":" + strconv.Itoa(height/divisor), nil
}
...
func resolutionFromVideoSize(value string) string {
	...
	best, distance := 480, abs(short-480)
	for _, candidate := range []int{720, 1080} {
		if current := abs(short - candidate); current < distance {
...
func compatibleVideoResponse(job mediadomain.Job, contentURL string) gin.H {
	response := gin.H{
		"id": job.ID, "task_id": job.ID, "object": "video", "model": job.Model,
		"status": "queued", "progress": max(0, min(99, job.Progress)),
		"created_at": job.CreatedAt.Unix(), "seconds": strconv.Itoa(job.Seconds), "size": job.Size,
	}
	switch job.Status {
