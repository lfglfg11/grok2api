...
type VideoInput struct {
	...
	PublicModel string
	// Compatibility selects the Sora/new-api compatible task ID and response shape.
	Compatibility bool
...
func (s *Service) runVideoJob(parent context.Context, job media.Job, route model.Route) {
	...
	for attempt := 0; attemptPolicy.allows(attempt); attempt++ {
		...
		result, err = s.generateVideoWithInputFallback(ctx, adapter, videoRequest, job.ID, hasLocalVideoReference(inputReferences))
